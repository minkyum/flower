...
f_mat <- matrix(NA,numPix,length(phenYrs))

for (i in 1:numPix){

  f_mat[i,] <- DoFlower(band1[i,],band2[i,],band3[i,],band4[i,],dates,phenYrs,params)
...
save(f_mat,file=paste0(ckPheDir,'/chunk_flower_',ckNum,'.rda'))
