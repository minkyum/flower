...
###############################
params <- fromJSON(file='/projectnb/modislc/users/mkmoon/sukyungkim/flowering_planet/input/PLCM_Parameters.json')
...
# dfileSR <- dfileSR[substr(basename(dfileSR),1,4)!="2023"]
# dfileUDM2 <- dfileUDM2[substr(basename(dfileSR),1,4)!="2023"]
# fileSR <- fileSR[substr(basename(dfileSR),1,4)!="2023"]
# fileUDM2 <- fileUDM2[substr(basename(dfileSR),1,4)!="2023"]

# dfileSR   <- list.files(path=imgDir,pattern=glob2rx('*20240*MS_SR*.tif'),recursive=T)
# fn <- sapply(strsplit(dfileSR,'/'),'[[',4)
# fnpart <- paste(sapply(strsplit(fn,'_'),'[[',1), 
#                 sapply(strsplit(fn,'_'),'[[',2),
#                 sapply(strsplit(fn,'_'),'[[',3),
#                 sapply(strsplit(fn,'_'),'[[',4), sep="_")
# dfileUDM2 <- c()
# for(i in 1:length(fnpart)) {
#   afn <- list.files(path=imgDir,pattern=glob2rx(paste0("*", fnpart[i], "*_udm2*",".tif*")),recursive=T)
#   dfileUDM2 <- c(dfileUDM2, afn)
# }
# fileSR   <- list.files(path=imgDir,pattern=glob2rx('*20240*MS_SR*.tif'),recursive=T,full.names=T)
# fileUDM2 <- c()
# for(i in 1:length(fnpart)) {
#   afn <- list.files(path=imgDir,pattern=glob2rx(paste0("*", fnpart[i], "*_udm2*",".tif*")),recursive=T,full.names=T)
#   fileUDM2 <- c(fileUDM2, afn)
# }
